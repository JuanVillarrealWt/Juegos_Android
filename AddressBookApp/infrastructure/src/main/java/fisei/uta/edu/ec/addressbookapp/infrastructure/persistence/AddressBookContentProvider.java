...
import fisei.uta.edu.ec.addressbookapp.infrastructure.R;
...
public class AddressBookContentProvider extends ContentProvider {
    ...
    static {
        // Uri for Contact with specified id ("contacts/#")
        uriMatcher.addURI(DatabaseDescription.AUTHORITY,
                Contact.TABLE_NAME + "/#", ONE_CONTACT);

        // Uri for Contacts table ("contacts")
        uriMatcher.addURI(DatabaseDescription.AUTHORITY,
    ...
    // required method: does not apply to this app
    @Override
    ...
    public Cursor query(Uri uri, String[] projection,
        ...
        // create SQLiteQueryBuilder to query contacts table
        SQLiteQueryBuilder queryBuilder = new SQLiteQueryBuilder();
        queryBuilder.setTables(Contact.TABLE_NAME);

        switch (uriMatcher.match(uri)) {
            case ONE_CONTACT:
                // match on contact with specified id
                queryBuilder.appendWhere(
                        Contact._ID + "=" + uri.getLastPathSegment());
                break;
            case CONTACTS:
                // match on all contacts
                break;
            default:
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (uriMatcher.match(uri)) {
            case ONE_CONTACT:
                // get from the uri the id of contact to delete
                String id = uri.getLastPathSegment();
